...
class AACT(object):
    ...
    def trial_query(self):
        ...
        return select([
            ...
            func.trim(func.regexp_replace(
                ...
                '(Inclusion|Major inclusion|INCLUSION)(.*?)(Exclusion|Major exclusion|EXCLUSION).*',
    ...
    def add_crc_classifier_to_query(self, query):
        studies = self.studies()
        keywords = self.keywords()
        conditions = self.conditions()
        nct_id = bindparam('nct_id')
    ...
    def add_immunotherapy_classifier_to_query(self, query):
        studies = self.studies()
        interventions = self.interventions()
        keywords = self.keywords()
    ...
    def limit_query_to_added_within(self, query, cutoff_days):
        studies = self.studies()
        return query.where(
            studies.first_received_date >= func.date(func.current_date() - cutoff_days)
        )
